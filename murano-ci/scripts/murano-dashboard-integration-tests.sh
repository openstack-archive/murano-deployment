...
sudo bash -x murano-ci/infra/deploy_component_new.sh $ZUUL_REF murano-dashboard $SERVER_IP $ZUUL_URL
sudo bash -x murano-ci/infra/configure_api.sh $SERVER_IP 5672 False murano$BUILD_NUMBER

cd murano-tests/muranodashboard-tests
sed "s%keystone_url = http://127.0.0.1:5000/v2.0/%keystone_url = http://$SERVER_IP:5000/v2.0/%g" -i config/config_file.conf
