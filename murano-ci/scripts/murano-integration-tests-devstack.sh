...
MURANO_APPS_LIST="io.murano.apps.apache.Apache io.murano.apps.apache.Tomcat io.murano.apps.PostgreSql io.murano.apps.linux.Telnet"
...
function prepare_murano_apps() {
    ...
    if [ "${PROJECT_NAME}" == 'murano' ]; then
        ...
        local app
        cd ${clone_dir}
        for app in $MURANO_APPS_LIST; do
