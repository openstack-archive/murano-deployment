...
function check_sys_packages()
{
    ...
    local retval=0
    local packages="qemu-kvm virt-manager virt-goodies virtinst bridge-utils libvirt-bin uuid-runtime samba samba-common"
...
function init()
{
    ...
    if [ ! -z "${vmswdir}" ]; then Config["vmsworkdir"]="${vmswdir}"; fi
    if [ ! -d "$WORK_DIR" ]; then
    ...
    Config["smbmountoptions"]="-o uid=$(id -u),gid=$(id -g)"
    ...
    if [ "${smbuser}" != "guest" ]; then
        if [ -f "${smbcredsfile}" ]; then rm -f "${smbcredsfile}" || exit $?; fi
        echo username="${smbuser}" > "${smbcredsfile}"
        echo domain="${smbhost}" >> "${smbcredsfile}"
        echo password="${smbpasswd}" >> "${smbcredsfile}"
        Config["smbmountoptions"]+=",credentials=${smbcredsfile}"
    else
...
function prepare_local_sambaserver()
{
    ...
    if [ "${Config["smblocalsetuprequired"]}" == true ]; then
        ...
        log "FYI: Linux smb conf part template could looks like:"
        cat "${START_DIR}/lib/add-to-smb.conf"
    fi
    return 0
}
# mounting cifs/smbfs
...
{
    mount | grep -q "${Config["smbmountpoint"]}" && umountremote
}
...
{
    local mirrordir="${WORK_DIR}/mirror"
    cp -r "${mirrordir}"/* "${Config["smbmountpoint"]}"/
}
...
function make_virtualfloppy()
{
    ...
    if [ ! -f "${vfloppy}" ]; then
        dd bs=512 count=2880 if=/dev/zero of="${vfloppy}" >> /dev/null || return $?
        ...
        sed "s/%_IMAGE_BUILDER_HOST_%/$(iniget "${CFG_FILE}" "samba" "host")/g" -i "${vms_path}"/mnt/floppy/autounattend.xml  || return $?
        sed "s/%_SHARE_PATH_%/$(iniget "${CFG_FILE}" "samba" "sharename")/g" -i "${vms_path}"/mnt/floppy/autounattend.xml  || return $?
        sleep 1
...
function start_win_vm()
{
    ...
    log " build started, reference image would be built as ${vm_img_ref_path}, check logfile - ${vm_build_log} !"
}
# cycled build
function process_windows()
{
    ...
    local rel_desc=''
    local rel_edits=''
    local rel_unatend_templ_prefix=''
    local rel_unatend_templ_dir=''
    ...
    do
        ...
        if [ "${rel_enabled}" == true ]; then
            ...
            rel_iso="$(iniget "${CFG_FILE}" "${release}" "iso")"
            rel_edits="$(iniget "${CFG_FILE}" "${release}" "editions")"
            rel_unatend_templ_prefix="$(iniget "${CFG_FILE}" "${release}" "unatend_template_prefix")"
            if [ -z "${rel_unatend_templ_prefix}" ]; then
                rel_unatend_templ_prefix="${release}"
            fi
            if [ -f "${rel_iso}" ]; then
                for rel_edition in ${rel_edits}
                do
                    rel_unatend_templ_dir="${START_DIR}/lib/windowssetup/unattend/${rel_unatend_templ_prefix}-${rel_edition}"
                    rel_vms_temp_dir="${Config["vmsworkdir"]}/${rel_unatend_templ_prefix}-${rel_edition}"
                    mkdir -p "${rel_vms_temp_dir}" || exit 2
                    make_virtualfloppy "${rel_unatend_templ_dir}" "${rel_vms_temp_dir}"
                    if [ "$?" -ne 0 ]; then
...
function usage()
{
    echo  "${0} --help - Help information
         ...
         --show-configured - Display chosen MS Windows releases to build([default]/available_win_versions config option value)
         --check-samba - Check and configure Samba server in case of [samba]/mode=local
         --run - Run automated image creation"
    echo "IMPORTANT!: To run this tool you need to setup SUDO WITHOUT PASSWORD PROMPT for your USER !!!
         run as root:
         echo \"${USER}    ALL = NOPASSWD: ALL\" > cat /etc/sudoers.d/${USER}
