...
sudo bash -x murano-ci/infra/deploy_component_new.sh $ZUUL_REF murano-dashboard 172.18.124.203 $ZUUL_URL
...
sed "s%keystone_url = http://127.0.0.1:5000/v2.0/%keystone_url = http://172.18.124.203:5000/v2.0/%g" -i config/config_file.conf
