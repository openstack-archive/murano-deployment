...
GIT_URL=${GIT_URL:-'https://git.openstack.org'}
...
case "${PROJECT_NAME}" in
    ...
    ;;
    *)
        echo "Project name '${GIT_PROJECT}' isn't supported yet."
        exit 1
    ;;
esac
#-----------------
...
function deploy_devstack() {
    ...
    if [ "${PROJECT_NAME}" == 'murano' ]; then
        ...
        git fetch ${GIT_URL}/${GIT_PROJECT} ${GIT_REF} && git checkout FETCH_HEAD
        popd
    fi
...
EOF
    ...
    case "${PROJECT_NAME}" in
        'murano')
            echo "MURANO_REPO=${GIT_URL}/${GIT_PROJECT}" >> local.conf
            echo "MURANO_BRANCH=${GIT_REF}" >> local.conf
        ;;
        'murano-dashboard')
            echo "MURANO_DASHBOARD_REPO=${GIT_URL}/${GIT_PROJECT}" >> local.conf
            echo "MURANO_DASHBOARD_BRANCH=${GIT_REF}" >> local.conf
        ;;
        'python-muranoclient')
            echo "MURANO_PYTHONCLIENT_REPO=${GIT_URL}/${GIT_PROJECT}" >> local.conf
            echo "MURANO_PYTHONCLIENT_BRANCH=${GIT_REF}" >> local.conf
        ;;
    esac
...
function adjust_time_settings(){
    ...
    sudo ntdpate -u ru.pool.ntp.org
}
#-------------------------------------------------------------------------------
