...
Log $logHorSeparator;
Log "Updating userdata.py for cloudbase-init";
& "xcopy.exe" "$srcScriptsPath\common\userdata.py" "C:\Program Files (x86)\Cloudbase Solutions\Cloudbase-Init\Python27\Lib\site-packages\cloudbaseinit\plugins\windows" /y
Log $logHorSeparator;
#Run sysprep and Co.
if($CanGo -eq 1)
{
	# For windows 2008 r2 x64
	New-ItemProperty HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce -Name "PostInstallSysprep" -Value "$env:WINDIR\System32\WindowsPowerShell\v1.0\powershell.exe -Command `"& $srcScriptsPath\w2k8r2-core\2.Start-Sysprep.ps1 -BatchExecution -CoreFunctionsPath $modulesDir\CoreFunctions\CoreFunctions.psm1`""
	Log $logHorSeparator;
