...
function make_img_with_murano_agent() {
    ...
    sudo apt-get -y install kpartx git qemu-utils python-pip
    ...
    for elements_dir in $(find ${agent_dir}/murano-apps -type d -iname elements); do
        cp -R ${elements_dir}/* ${agent_dir}/elements/
    done
    ...
    ${agent_dir}/diskimage-builder/bin/disk-image-create vm ${DISTR_NAME} \
        murano-agent docker kubernetes -o ${DISTR_NAME}${BUILD_NUMBER}-murano-agent.qcow2
    popd
