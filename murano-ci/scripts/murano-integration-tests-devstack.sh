...
function run_tests() {
    ...
    TESTS_STARTED_AT=($(TZ=Europe/Moscow date +'%Y-%m-%d %H:%M:%S'))
    case "${PROJECT_NAME}" in
        'murano')
            $NOSETESTS_CMD -s -v \
                --with-xunit \
...
#            $NOSETESTS_CMD -s -v sanity_check.py:TestSuiteFields || retval=$?
    ...
    TESTS_FINISHED_AT=($(TZ=Europe/Moscow date +'%Y-%m-%d %H:%M:%S'))
...
function collect_openstack_logs() {
    ...
    ssh $KEYSTONE_URL BUILD_TAG=${BUILD_TAG} ./cut-logs.sh ${TESTS_STARTED_AT[0]} ${TESTS_STARTED_AT[1]} ${TESTS_FINISHED_AT[0]} ${TESTS_FINISHED_AT[1]} heat neutron
    scp -r $KEYSTONE_URL:~/output/${BUILD_TAG}/* ${WORKSPACE}/artifacts/openstack
    ssh $KEYSTONE_URL rm -rf ~/output/${BUILD_TAG}
    set -o errexit
}
