...
function make_img_with_murano_agent() {
    ...
    git clone "${ZUUL_URL}/${PROJECT_NAME_REAL}" "${agent_dir}/murano-agent"

    pushd "${agent_dir}/murano-agent"
    git fetch "${ZUUL_URL}/${PROJECT_NAME_REAL}" "${ZUUL_REF}" && git checkout FETCH_HEAD
    popd
    ...
    if [[ "${DISTR_NAME}" == "debian" ]]; then
        disk-image-create vm "${DISTR_NAME}" murano-agent-debian -o "${DISTR_NAME}${BUILD_NUMBER}-murano-agent.qcow2"
    else
        disk-image-create vm "${DISTR_NAME}" cloud-init-datasources murano-agent -o "${DISTR_NAME}${BUILD_NUMBER}-murano-agent.qcow2"
    fi
    popd
