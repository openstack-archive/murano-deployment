...
python murano-ci/infra/RabbitMQ.py -username murano$BUILD_NUMBER -vhostname murano$BUILD_NUMBER -rabbitmq_url 172.18.124.201:15672

sudo bash -x murano-ci/infra/deploy_component_new.sh $ZUUL_REF murano-api noop $ZUUL_URL
sudo bash -x murano-ci/infra/configure_api.sh 172.18.124.201 5672 False murano$BUILD_NUMBER
...
sed -i "s/uri = http:\/\/127.0.0.1:5000\/v2.0\//uri = http:\/\/172.18.124.201:5000\/v2.0\//" etc/tempest.conf
sed -i "s/admin_username = admin/admin_username = ci-user/" etc/tempest.conf
sed -i "s/admin_password = secret/admin_password = swordfish/" etc/tempest.conf
sed -i "s/admin_tenant_name = admin/admin_tenant_name = ci/" etc/tempest.conf
...
if [ $? == 1 ]
then
   python $WORKSPACE/murano-ci/infra/RabbitMQ.py -username murano$BUILD_NUMBER -vhostname murano$BUILD_NUMBER -action delete -rabbitmq_url 172.18.124.201:15672
   exit 1
fi
