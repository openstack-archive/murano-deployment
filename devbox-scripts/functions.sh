...
function configure_murano {
    ...
    MURANO_DASHBOARD_CACHE=$MURANO_DASHBOARD_CACHE MURANO_AUTH_HOST=$MURANO_AUTH_HOST /usr/bin/modify-horizon-config.sh install

    mkdir -p "$MURANO_DASHBOARD_CACHE"
    chown -R $MURANO_USER:$MURANO_GROUP "$MURANO_DASHBOARD_CACHE"
    chmod -R 755 "$MURANO_DASHBOARD_CACHE"

    python "$HORIZON_DIR/manage.py" collectstatic --noinput
    python "$HORIZON_DIR/manage.py" syncdb --noinput
    #--------------------------------
