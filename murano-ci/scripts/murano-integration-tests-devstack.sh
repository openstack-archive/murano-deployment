...
function make_img_with_murano_agent() {
    ...
    sudo git clone https://git.openstack.org/openstack/diskimage-builder.git \
        ${agent_dir}/diskimage-builder
    sudo git clone ${ZUUL_URL}/${ZUUL_PROJECT} ${agent_dir}/murano-agent

    pushd ${agent_dir}/murano-agent
    sudo git fetch ${ZUUL_URL}/${ZUUL_PROJECT} ${ZUUL_REF} && git checkout FETCH_HEAD
    popd

    export ELEMENTS_PATH=${agent_dir}/murano-agent/contrib/elements
    sudo ${agent_dir}/diskimage-builder/bin/disk-image-create vm ${DISTR_NAME} \
