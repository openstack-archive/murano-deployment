    ...
    for app_name in $murano_components ; do
        ...
        if [ -z $return_app_name ] ; then
            ...
            if [ -n "$REMOTE_COMMIT" ] ; then
                git fetch "$git_repo" "$REMOTE_COMMIT" && git checkout FETCH_HEAD
            fi

            log "* Switched to '$REMOTE_BRANCH' / '$REMOTE_COMMIT':"
            log "----- ----- ----- ----- -----"
            log "(git log -1):"
            git log -1
            log "===== ===== ===== ===== ====="
