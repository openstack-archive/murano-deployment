...
case "${PROJECT_NAME}" in
    ...
    ;;
    'murano-agent')
        MURANO_PROJECT_DIR="${STACK_HOME}/murano"
        PROJECT_TESTS_DIR="${MURANO_PROJECT_DIR}/murano/tests/functional"
    ;;
    *)
...
function make_img_with_murano_agent() {
   ...
   mkdir -p ${agent_dir}
   git clone https://git.openstack.org/openstack/diskimage-builder.git ${agent_dir}/diskimage-builder
   ...
   ${agent_dir}/diskimage-builder/bin/disk-image-create vm ${DISTR_NAME} murano-agent -o ${DISTR_NAME}${BUILD_NUMBER}-murano-agent.qcow2
   pushd "${STACK_HOME}/devstack"
   source openrc ${ADMIN_USERNAME} ${ADMIN_TENANT}
   popd
   glance image-create --name ${DISTR_NAME}${BUILD_NUMBER} --disk-format qcow2 --container-format bare \
...
function remove_image_with_murano_agent() {
   ...
   source openrc ${ADMIN_USERNAME} ${ADMIN_TENANT}
   popd
   for i in `glance image-list | get_field 2`; do if [ "${dist_name}${BUILD_NUMBER}" == "$i" ]; then `glance image-delete $i`; fi; done;
