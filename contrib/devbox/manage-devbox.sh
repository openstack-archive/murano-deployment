...
function create_venv {
    ...
    pushd ${path}
    tox -r -e venv -- python -c "print('VENV installed.')"
    popd
...
function import_app {
    ...
    echo ''
    if [[ -f "${DEST}/murano-app-incubator/${path}/manifest.yaml" ]]; then
        app_path="${DEST}/murano-app-incubator/${path}"
    elif [[ -f "${path}/manifest.yaml" ]]; then
    ...
    if [[ -n "${app_path}" ]]; then
        ...
        pushd ${DEST}/murano
        tox -e venv -- murano-manage --config-file ./etc/murano/murano.conf import-package "${app_path}" --update
        popd

        echo '... done'
    else
        echo "No Murano Application found using '${path}'."
    fi
}
...
COMMANDS
    ...
    prep
        Prepare devbox for launching Murano services.

    import <path>
        Import Murano Application from <path>.
EOF
}
...
case $1 in
    'start')
        ...
        screen_service 'murano-api' "${DEST}/murano" 'tox -e venv -- murano-api --config-file ./etc/murano/murano.conf'
        screen_service 'murano-engine' "${DEST}/murano" 'tox -e venv -- murano-engine --config-file ./etc/murano/murano.conf'
