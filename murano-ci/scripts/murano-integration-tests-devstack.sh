...
function prepare_tests() {
    ...
    case "${PROJECT_NAME}" in
        ...
        ;;
        'murano-dashboard')
            local tests_config="${PROJECT_TESTS_DIR}/config/config.conf"
            local section_name='murano'
        ;;
        'python-muranoclient')
            local tests_config="${PROJECT_TESTS_DIR}/engine/config.conf"
    ...
    iniset "${section_name}" 'keystone_url' "$(shield_slashes http://${KEYSTONE_URL}:5000/v2.0/)" "${tests_config}"
    iniset "${section_name}" 'horizon_url' "$(shield_slashes http://${found_ip_address}/)" "${tests_config}"
    iniset "${section_name}" 'murano_url' "$(shield_slashes http://${found_ip_address}:8082/v1/)" "${tests_config}"
    iniset "${section_name}" 'user' "${ADMIN_USERNAME}" "${tests_config}"
    iniset "${section_name}" 'password' "${ADMIN_PASSWORD}" "${tests_config}"
    iniset "${section_name}" 'tenant' "${ADMIN_TENANT}" "${tests_config}"
    iniset "${section_name}" 'linux_image' "${LINUX_IMAGE}" "${tests_config}"
    iniset "${section_name}" 'auth_url' "$(shield_slashes http://${KEYSTONE_URL}:5000/v2.0/)" "${tests_config}"
