...
def remove_rabbit_endpoint(vhost_name,vhost_owner):
    ...
    for attempt in range(retries):
        try:
            ...
            unmute_stdout()
            print("User '%s' deletion fails or user doesn't exists, making one more attempt '%s'" % (vhost_owner, (attempt+1)))
    ...
    for attempt in range(retries):
        try:
            ...
            unmute_stdout()
            print("Error occurred:'%s', making one more attempt '%s'" % (err, (attempt+1)))
            time.sleep(ret_interval)
        else:
            unmute_stdout()
