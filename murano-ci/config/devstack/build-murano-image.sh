...
case ${base_image_type} in
    ...
    ;;
    'ubuntu')
        base_image_name='ubuntu-12.04-server-cloudimg-amd64-disk-1'
        instance_name='ubuntu-12.04-server-cloudimg-amd64-murano'
    ;;
    *)
        die "Image type '${base_image_type}' is not supported."
    ;;
esac
...
runcmd:
 ...
 - "cd /opt/git && git clone https://github.com/stackforge/murano-agent"
 - "cd /opt/git/murano-agent && git checkout release-0.5"
 - "cd /opt/git/murano-agent/python-agent && bash setup.sh install"
 - "rm -rf /opt/git"
 - "rm -rf /var/lib/cloud/instance"
 - "rm -rf /var/lib/cloud/instances/*"
 - "rm -rf /var/lib/cloud/sem/*"
...
while [ ${timeout} -gt 0 -a "${instance_status}" != 'SHUTOFF' ]; do
    ...
    echo "Status of instance ${instance_name}(${instance_id}) is '${instance_status}'"
done
