...
if [ $distro_based_on == "redhat" ]; then
    yum update -y || exit $?
    yum install -y rabbitmq-server || exit $?
else
    apt-get update || exit $?
    apt-get install -y rabbitmq-server || exit $?
fi
...
function handle_rabbitmq()
{
    ...
    case $action in
        add)
            $PYTHON_CMD ${CI_ROOT_DIR}/infra/RabbitMQ.py -username murano$BUILD_NUMBER -vhostname murano$BUILD_NUMBER -rabbitmq_url localhost:55672 -action create
            if [ $? -ne 0 ]; then
